...
function editor_small_onload(resource){
    ...
    $.ajax({
        type: "GET",
        dataType: 'json',
        url: gpath + "/admin/sub_category/" + $.cookie('editor'),
...
function catagory_commit_s(){
    ...
    switch (btn) {
        case "添加小类":
            ...
            url += '/admin/sub_category/post';
            request_type = "POST";
            break;
        case "保存":
            url += '/admin/sub_category/' + sub_category_id;
            request_type = "PUT";
    }
    $.ajax({
        type: request_type,
...
function remove_small(){
    ...
    if (confirm(msg) == true) {
        $.ajax({
            type: "DELETE",
            dataType: 'json',
            url: gpath + "/admin/sub_category/" + $('.small_input').attr('id'),
            success: function (result) {
                if (result.resCode == 0) {
