...
function editor_big_onload(resource){
    ...
    $.ajax({
        type: "GET",
        dataType: 'json',
        url: gpath + '/admin/category/' + $.cookie('big_category'),
            ...
            switch (btn) {
                case "添加大类":
                    if (result.resCode == 0) {
                        ...
                        $('.category').empty();
                        $('#xt_id').val(result.id);
                        image_upload($("#xt_id"), "tb_form", "/admin/category_picture_upload/");
                        image_upload($("#xt_id"), "tp_form", "/admin/category_picture_upload/");
                        big_list();
                    } else {
...
function remove_catagory(){
    ...
    if (confirm(msg) == true) {
        $.ajax({
            type: "DELETE",
            dataType: 'json',
            url: gpath + "/admin/category/" + $('#xt_id').val(),
            success: function (result) {
                if (result.resCode == 0) {
