...
function detail_addition_onload(resource) {
    ...
    $.ajax({
        type: "GET",
        dataType: 'json',
        url: gpath + "/general/business/" + $.cookie('detail_addition2'),
        success: function (result) {
            if (result.resCode == 0) {
                $('.one input').attr('id', $.cookie('detail_addition2'));
...
function detail_nochange_addition_onload(resource) {
    var tit = '<i class="iconfont icon-shengqian"></i>事项';
    $('.config-title h1').empty().append(tit);
    $('.addition_btn').empty();
...
function add_addition() {
    ...
    switch (btn) {
        case "添加事项":
            url += '/general/business/post';
            request_type = "POST";
            // alert(123)
            break;
        case "保存事项":
            url += '/general/business/' + $('.one input').attr('id');
            request_type = "PUT";
    }
    $.ajax({
        type: request_type,
...
function remove_addition() {
    ...
    if (confirm(msg) == true) {
        $.ajax({
            type: "DELETE",
            dataType: 'json',
            url: gpath + "/general/business/" + $('.one input').attr('id'),
            success: function (result) {
                if (result.resCode == 0) {
